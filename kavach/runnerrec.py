    ...
    try:
        while True:
            ...
            if not ret:
                ...
                print(f"\n✅ End of video reached. Processed {processed_count} frames successfully.")
                break
            ...
            if write_success:
                ...
                print(f"\n✅ Frame {frame_count}/{total_frames} saved at {video_timestamp:.1f}s ({file_size} bytes)")
            else:
                    ...
                    if r.status_code == 200:
                        ...
                        if task_name == "person_detection":
                            ...
                            if len(count_history) >= 3:  # Show smoothed after 3 frames
                                print(f"  ✅ Counting  | Count: {smoothed_count} (smoothed) | Raw: {count} | Avg Conf: {conf:.2f} | Latency: {elapsed}ms")
                            else:
                            ...
                            if annotated_uri:
                                print(f"  🎨 Annotated image available at: {annotated_uri}")
        ...
        print(f"\n\n{'='*60}")
        print(f"🛑 Video processing stopped by user")
        print(f"Processed frames: {processed_count}/{total_frames}")
        print(f"{'='*60}\n")
    
    finally:
        cap.release()
        print(f"\n✅ Video processing complete. Total frames processed: {processed_count}")
...
Examples:
  ...
  python runnerrec.py --list-tasks
    ...
    if available_tasks:
        ...
        if invalid_tasks:
            print(f"\n⚠️  Warning: Task(s) '{', '.join(invalid_tasks)}' not found in adapter capabilities")
